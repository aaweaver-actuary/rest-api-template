...
from utils import ENDPOINT, get_price, DOCS_ENDPOINT
...
with col2:
    mod_factor = st.number_input(
        "Mod Factor", min_value=0.9, max_value=2.0, format="%.2f", value=1.5
    )
...
if st.button("Calculate the rate!", type="primary"):
    try:
        ...
        # st.write(rate)
        st.success(f"Calculated rate: {rate}")
    except Exception as e:
        st.error(f"Error: {e}")
else:
    st.info("Not yet calculated :(")

st.markdown(f"[See API Docs ->]({DOCS_ENDPOINT})")
